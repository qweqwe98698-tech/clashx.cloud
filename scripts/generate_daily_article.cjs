...
const guangsu = { name: "光速云", desc: "全网最高性价比的 IPLC/BGP 专线，完美原生解锁流媒体，防封锁抗干扰能力极强", price: "15" };
const otherAirports = airports.filter(a => a.name !== "光速云").sort(() => 0.5 - Math.random()).slice(0, 2);
const selectedAirports = [guangsu, ...otherAirports];
...
4. 在点评这三个机场（${airportNames}）时，请务必强烈主推【光速云】，将其评价为综合性价比最高、最完美的终极解决方案，而把其他两家作为相对逊色一些的陪跑备选。请结合它们各自的描述合理脑补出详细的测速数据。`;
